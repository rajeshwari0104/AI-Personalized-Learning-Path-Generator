...
export default function SharedRoadmap() {
  ...
  if (resource.resource_type === 'article') {
    // Rotates between top learning sites based on title length (simple hash)
    const sites = [
      `https://www.freecodecamp.org/news/search/?query=${q}`,
      `https://www.geeksforgeeks.org/search/?q=${q}`,
      `https://realpython.com/search?q=${q}`,
      `https://developer.mozilla.org/en-US/search?q=${q}`,
    ];
    return sites[resource.title.length % sites.length];
  }
  if (resource.resource_type === 'exercise') {
    const platforms = [
      ...
      `https://www.codecademy.com/search?query=${q}`,
    ];
    return platforms[resource.title.length % platforms.length];
  }
  if (resource.resource_type === 'book') {
    return `https://www.goodreads.com/search?q=${q}`;
  }
  return `https://www.google.com/search?q=${qtutorial}`;
};
  if (loading) return (
    <div style={s.loading}>
      <style>{css}</style>
...
const css = `
  ...
  * { font-family: 'Space Grotesk', sans-serif; box-sizing: border-box; }
  ...
  .cta-btn:hover { transform: translateY(-3px); box-shadow: 0 15px 40px rgba(168,85,247,0.5); }
`;
...
const s = {
  ...
  header: { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: '2rem', marginBottom: '1.5rem' },
  ...
  weekTabActive: { background: 'rgba(168,85,247,0.15)', border: '1px solid rgba(168,85,247,0.35)', color: '#c084fc' },
  weekCard: { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 24, padding: '2rem', marginBottom: '1.5rem' },
  ...
  cta: { background: 'linear-gradient(135deg, rgba(168,85,247,0.15), rgba(236,72,153,0.1))', border: '1px solid rgba(168,85,247,0.25)', borderRadius: 24, padding: '2.5rem', textAlign: 'center' },
  ...
  ctaBtn: { padding: '0.9rem 2.5rem', background: 'linear-gradient(135deg, #a855f7, #ec4899)', border: 'none', borderRadius: 100, color: 'white', fontSize: 16, fontWeight: 700, cursor: 'pointer' },
};
